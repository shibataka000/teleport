...
func setupWithOptions(t *testing.T, opts appTestOptions) *pack {
	...
	rcConf.Console = nil
	rcConf.Log = log
	rcConf.DataDir, err = os.MkdirTemp("", "cluster-"+p.rootCluster.Secrets.SiteName)
	require.NoError(t, err)
	...
	lcConf.Console = nil
	lcConf.Log = log
	lcConf.DataDir, err = os.MkdirTemp("", "cluster-"+p.leafCluster.Secrets.SiteName)
	require.NoError(t, err)
	...
	laConf.Console = nil
	laConf.Log = log
	laConf.DataDir, err = os.MkdirTemp("", "app-server-"+p.leafCluster.Secrets.SiteName)
	require.NoError(t, err)
...
func (p *pack) startRootAppServers(t *testing.T, count int, extraApps []service.App) []*service.TeleportProcess {
	...
	for i := 0; i < count; i++ {
		...
		raConf.Console = nil
		raConf.Log = log
		raConf.DataDir, err = os.MkdirTemp("", fmt.Sprintf("app-server-%d-%s", i, p.rootCluster.Secrets.SiteName))
		require.NoError(t, err)
