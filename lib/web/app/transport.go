...
type transport struct {
	...
	// servers servers that the transport can connect to organized in a map
	// where the key is the server ID and the value is the `types.AppServer`.
	servers *sync.Map
}
	...
	t.servers.Range(func(serverID interface{}, appServerInterface interface{}) bool {
		...
		if err != nil {
			...
			if trace.IsConnectionProblem(err) {
				t.c.log.Warnf("Failed to connect to application server \"%d\": %v.", serverID, err)
