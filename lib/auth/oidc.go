...
func (a *Server) getClaims(oidcClient *oidc.Client, connector types.OIDCConnector, code string) (jose.Claims, error) {
	...
	if connector.GetIssuerURL() == teleport.GSuiteIssuerURL && (connector.GetGoogleServiceAccountURI() != "" || connector.GetGoogleServiceAccount() != "") {
		...
		domain, exists, err := claims.StringClaim("hd")
		if err != nil || !exists {
			return nil, trace.BadParameter("no hd (hosted domain) in oauth claims for Google Workspace account")
		}
		...
		// The "Directory API" needs admin delegation (see
		// https://developers.google.com/admin-sdk/directory/v1/guides/delegation
		// and
		// https://developers.google.com/identity/protocols/oauth2/service-account#delegatingauthority )
		// so we use the presence of google_admin_email to switch between
		// "Directory API" and "Cloud Identity API"
		...
		if connector.GetGoogleAdminEmail() != "" {
			gsuiteGroups, err = groupsFromGsuiteDirectory(a.closeCtx, credentials, email, domain)
			if err != nil {
				return nil, trace.Wrap(err)
			}
		} else {
...
func groupsFromGsuiteDirectory(ctx context.Context, credentials *google.Credentials, email string, domain string) ([]string, error) {
...
func groupsFromGsuiteCloudidentity(ctx context.Context, credentials *google.Credentials, email string) ([]string, error) {
	...
	err = service.Groups.Memberships.SearchTransitiveGroups("groups/-").
		Query(fmt.Sprintf("member_key_id == %q && 'cloudidentity.googleapis.com/groups.discussion_forum' in labels", email)).
